/*Author: Yanhan Lyu*/
/*an array to hold the transformed varings*/
double varyAll[renVARYDIMBOUND * renVERTNUMBOUND];
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[], 
		texTexture *tex[]) {
	/* check if attrDim in mesh is equal to that in ren*/
	if (mesh->attrDim != ren->attrDim) {
		printf("Error, the attrDim in mesh and attrDim in renderer are different.\n");
		printf("renderer, mesh, %d, %d\n ", ren->attrDim, mesh->attrDim);
	} else {
		for (int i = 0; i < mesh->vertNum; i++) {
			/*get the ith vertex*/
			double *vertex = meshGetVertexPointer(mesh, i);
			ren->transformVertex(ren, unif, vertex, &varyAll[i*ren->varyDim]);
		}
		for (int k = 0; k< mesh->triNum; k++) {
			int *triangle = meshGetTrianglePointer(mesh, k);
			/*get the ith triangle*/
			double *varyA = &(varyAll[triangle[0]*ren->varyDim]);
			double *varyB = &(varyAll[triangle[1]*ren->varyDim]);
			double *varyC = &(varyAll[triangle[2]*ren->varyDim]);
