...
void texSample(texTexture *tex, double s, double t) {
	...
	if (tex->filtering == texQUADRATIC) {
        ...
        /*Use the aux to store additional information*/
        /* look up the texture at (ceil(i),ceil(v))*/
		texGetTexel(tex, ceil(u), ceil(v), tex->sample);
        vecScale(tex->texelDim, uFrac*vFrac, tex->sample, tex->aux);
        /* look up the texture at ceil(u), floor(v))*/
		texGetTexel(tex, ceil(u), floor(v), tex->sample);
        vecScale(tex->texelDim, uFrac*(1-vFrac), tex->sample, tex->sample);
        vecAdd(tex->texelDim, tex->sample, tex->aux,tex->aux);
        /* look up the texture at (floor(u), ceil(v))*/
		texGetTexel(tex, floor(u), ceil(v), tex->sample);
        vecScale(tex->texelDim, (1-uFrac)*vFrac, tex->sample, tex->sample);
        vecAdd(tex->texelDim, tex->sample, tex->aux,tex->aux);
		/* look up the texture at (floor(u), floor(v))*/
		texGetTexel(tex, floor(u), floor(v), tex->sample);
        vecScale(tex->texelDim,  (1-uFrac)*(1-vFrac), tex->sample, tex->sample);
        vecAdd(tex->texelDim, tex->sample, tex->aux,tex->aux);
        /* let sample hold the calculated pixels*/
        vecCopy(tex->texelDim, tex->aux, tex->sample);
